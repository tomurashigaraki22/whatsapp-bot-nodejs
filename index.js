...
client.on("message_create", async (message) => {
	...
	} else if (messageBody.startsWith('/question')) {
		const quest = messageBody.split(' ');
		const quest1 = quest.slice(1).join(' '); // Join the elements starting from index 1 to the end
	
		// Call the question function and wait for the response using await
		try {
			const response = await question(quest1);
			console.log(response); // Handle the response from OpenAI as needed
